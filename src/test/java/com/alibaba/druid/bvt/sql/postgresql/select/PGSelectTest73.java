...
public class PGSelectTest73 extends TestCase {
    public void test_0() throws Exception {
        ...
        assertEquals("SELECT a.*, AVG(a.nv_roe_lag) OVER () AS nv_roe_avg, AVG(a.idx_roe_lag) OVER (ORDER BY a.stat_date) AS idx_roe_avg\n" +
                ...
                "\t, stddev_samp(a.nv_roe_lag) OVER (ORDER BY a.stat_date) AS nv_stddev, stddev_samp(a.idx_roe_lag) OVER (ORDER BY a.stat_date) AS idx_stddev\n" +
                "\t, stddev_samp(a.if300_roe_lag) OVER (ORDER BY a.stat_date) AS if300_stddev, stddev_samp(a.ih50_roe_lag) OVER (ORDER BY a.stat_date) AS ih50_stddev\n" +
